...
You can move the robot to a specific position with a specific speed and acceleration.
You can also get the current position of the robot.
Don’t change them!!!
...
# Get the current joint position or find it on the UR5e robot controller terminal interface
robot.get_actual_pose()
print("Current Cartesian Pose:", actual_pose)
...
class UR5eRobot:
    ...
    #     # Validate input
    #     if not all(isinstance(val, (int, float)) for val in [x, y, z, rx, ry, rz]):
    #         rospy.logerr("Invalid input: All position and rotation values must be numbers.")
    #         return
    ...
    #     self.execute_cartesian_trajectory(pose_list)
