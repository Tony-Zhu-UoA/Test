...
from function_library.basis_function.tool_detect_pick_top_camera import AzureKinectTopCamera
...
if __name__ == "__main__":
    ...
    model_path = "/home/zzq/hrc_352lab/src/lab_codes/scripts/function_library/basis_function/model_lib/voice.pt" # Path to your YOLOv8 model
    camera = AzureKinectTopCamera(model_path)
    ...
    while True:
        try:
            ...
            if tool_id not in [1, 2, 4]:
                continue
            if not camera.robot_pick_top_camera(0.133, tool_id):
                rospy.loginfo("Failed to pick up tool. Retrying...")   
            else:
                gripper.close_with_force(230.0)
                time.sleep(3)  
                list =[]
                list.append([0.7, 0.4, 0.3335, 2.231, -2.216, 0.0])
                robot.execute_cartesian_trajectory(list)
                attempt = 0
                # Wait for the user to take the tool
                while attempt < 20:
                    if gripper.gripper_is_open():
                        rospy.loginfo("The tool is taken.")
                        break
                    else:
                        rospy.loginfo("Please take your tool")
                        time.sleep(3)
                    attempt += 1
                robot.go_home()
